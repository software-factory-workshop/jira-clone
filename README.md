...
The cockpit starts and resumes durable Eve investigations and keeps request drafts in shared, versioned cockpit storage. “Create issue in GitHub” opens a prefilled issue; the user decides whether to submit it. “Go!” saves an explicit operator idea and starts durable delivery; task mining is an optional evidence-first path that can also admit a work order. Draft storage is shared team state, with version checks for concurrent edits. The task-mining station uses Vercel Connect for fixed-repository context when installed.
