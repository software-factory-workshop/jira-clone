...
Start with [the current goal](context/goal.md), then use [the project map](context/project-map.md) to inspect the relevant code and evidence. Read [work already underway](context/work-in-progress.md) alongside current GitHub issues before proposing work. Every station also follows the [shared agent quality contract](policies/agent-quality.md).
...
GitHub credentials come from Vercel Connect (`github/jira-clone`). The experiment reads only this repository. Mining keeps bounded GET access. The worker publishes through a separate host tool restricted to one draft PR in this repository; its sandbox receives no credential. Stations have no merge or factory-policy activation capability. The outer Vercel Workflow returns blocking findings to the original worker and requests another independent review. After exact review, check and trusted changed-file gates pass, the host delivery driver can merge only narrowly defined low-risk changes, even when browser evidence is missing. Broad or elevated merges remain human-controlled. See `docs/delivery-loop.md` for the policy and recovery protocol.
