...
The root and worker share ceilings of 1,000,000 cumulative input tokens, 32,000 output tokens and $0.20 model cost. These root ceilings also apply to mining; the reviewer keeps its narrower child limits. A child receives the remaining parent allowance, so raising only a worker limit does not raise the effective job budget. Sandbox compute is billed separately.
