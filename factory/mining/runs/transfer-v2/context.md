...
Hypotheses to test: an explicit source inventory reduces false missing-work proposals, and checking quoted details reduces transcription errors. A fresh same-prompt replay and a differently worded investigation should test these lessons. Neither observation proves broad model reliability.
...
- Read `factory/context/goal.md` for current direction and `factory/context/project-map.md` for source navigation. The deployed apps remain stage zero. A developer-run, read-only task-mining experiment is authorized; a deployed Eve workflow and product implementation are separate steps.
