...
Delivery observation is bounded and resumable. The observer captures one Eve stream tail, reads at most 30,000 events from index zero, and applies a 10-second idle deadline to each read rather than a total wall-clock deadline. It never accepts a partial prefix as a verdict: early close, an idle timeout, an invalid tail, or an over-limit history returns a typed `observation_*` failure. The delivery records the exact phase in `failedPhase`, transitions to `blocked`, releases its CAS lease, and returns `503` with a `/resume` action. Resume re-observes the same session and owner; it does not create a replacement worker. A healthy long stream can therefore exceed ten seconds overall as long as each read makes progress.

Delivery failures have distinct transport and state semantics:

| Failure | HTTP | Delivery effect | Recovery |
| --- | ---: | --- | --- |
| `observation_timeout`, `observation_partial`, `observation_limit` | 503 | `blocked`, with `failedPhase` preserved | `/resume` the same owner/session |
| `provider_unavailable` | 502 (or upstream 429) | phase is preserved; the lease is released, but no false `blocked` transition is written | retry `/advance` |
| `provider_auth` | 401/403 | phase is preserved; no work result is accepted | repair provider credentials/authentication |
| `invalid_request` | 400 | no delivery failure transition | correct the request |
| `needs_revision` / `stale_head` / `target_advanced` | 409 | `needs_revision` | refresh the original owner and republish |
| `target_closed` | 409 | `human_review` | make an explicit target decision |

The response body carries `{ error: { code, kind, message, retryable } }`, plus the delivery identity and action when state was already claimed. Route authentication failures still return the auth response before a delivery is claimed. Cancellation remains intent-first: a late observer result cannot overwrite the cancelled projection.

`working` and `revising` wait for successful host `publish_work` output, then `reviewing` waits for host `record_review`. Model prose cannot advance the loop. The host rechecks current PR head and target before dispatching review and accepting its result. Changed refs yield `needs_revision` with an explicit original-owner refresh request; closed or retargeted PRs yield `human_review` with an explicit target decision. Blocking findings automatically return to the same owner, up to the explicitly configured `maxRevisions` (default 3); this controls repeated repair attempts, not model token budgets. After review, the host merge policy checks the exact PR head and target, independent verification, GitHub checks and changed files. Small documentation changes outside factory policy, and narrowly defined cosmetic CSS changes, may merge automatically. Missing browser evidence alone does not block these low-risk merges. Changes outside that allowlist, blocking findings, missing checks or changed revisions require human review. Agents cannot grant themselves merge authority. GitHub atomically checks the candidate SHA at merge; the target SHA is checked immediately beforehand but is not an atomic merge precondition. Main can advance in that gap. The narrow low-risk policy accepts this limitation; it is not an exact-target merge queue.
