...
Delivery observation fails closed: the outer observer reads the full Eve stream up to the captured tail and never accepts a partial prefix as a verdict. A timed-out observation keeps the delivery in its phase with a `Session observation timed out; no partial result accepted.` error; retrying `advance` re-observes the same session rather than replacing the worker.
