...
import { REST_BOUNDARY, authorizeBearerRead, restBearerIdentity, restProjectStatuses } from "../../../../../../utils/jiraRest";
 ...
 * status list, not a verified transition graph. A present
 * `Authorization: Bearer` demo OAuth token is enforced fail-closed (reads
 * require the `read` scope) and never falls through to the demo read.
 * Never writes.
 */
