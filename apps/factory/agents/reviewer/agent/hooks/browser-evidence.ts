...
import {MAX_REVIEW_FRAME_DATA_URL_LENGTH,reviewBrowser} from '../../../../runtime/lib/review-browser';
...
export default defineHook({events:{'action.result'(event,ctx){
 ...
 if(tree){
  ...
  active.update(()=>origin);reviewBrowser.update(s=>{const prior=s.observations[origin];const retained=prior?.route===route?prior:undefined;return{...s,observations:{...s.observations,[origin]:{origin,headSha,sessionId:ctx.session.id,snapshot:true,interaction:retained?.interaction||false,keyboard:retained?.keyboard||false,screenshot:retained?.screenshot||false,afterInteraction:!!retained?.interaction&&!!retained?.keyboard,route,frames:retained?.frames,eventIds:[...(retained?.eventIds||[]),event.meta.id].slice(-50)}}};});return;
 }
 ...
 reviewBrowser.update(s=>{const prior=s.observations[origin];if(!prior)return s;const phase=prior.interaction||prior.keyboard?'after':'before';const frame=captured?{phase,url:prior.origin+(prior.route||'/'),route:prior.route||'/',dataUrl,capturedAt:new Date().toISOString(),eventId:event.meta.id}:undefined;return{...s,observations:{...s.observations,[origin]:{...prior,interaction:prior.interaction||interaction,keyboard:prior.keyboard||keyboard,screenshot:prior.screenshot||captured,afterInteraction:interaction||keyboard?false:prior.afterInteraction,frames:frame?{...prior.frames,[phase]:frame}:prior.frames,eventIds:[...prior.eventIds,event.meta.id].slice(-50)}}};});
}}});
