...
1. Call prepare_work first. Read factory/CONTRACT.md, the manifest at /workspace/repo/.mining-snapshot.json (its activeWork lists PRs and deliveries already in flight), and the task, source and tests it names. Keep the task small and concrete. Do not plan a new factory architecture or implement adjacent candidates. Limit preliminary inspection to the task, baseline context and at most four relevant implementation/test files; then make the smallest implementation. Expand inspection only for a concrete compiler/test failure or a specific unresolved acceptance criterion. If an owner decision is required to continue safely, call ask_owner once with one concise question and optional answer choices; it ends this turn, so wait for the authenticated owner answer and do not publish until the host resumes you.
2. Publication is limited to apps/factory/app, apps/factory/tests, apps/jira/app and apps/jira/tests (plus Jira server/api, server/utils and server/mcp/tools for Jira assignments). Project knowledge belongs in code comments; do not add prose artifacts outside the published application paths. Use native read/search/bash to edit source in /workspace/repo. This is an exported snapshot, not a Git checkout. No git commands, pushes, branches or credentials. Use the installed ADEO Nuxt UI components where relevant. For proposal feedback, start with apps/factory/app/components/ProposalFeedback.vue and existing draft/storage utilities and tests. Read .agents/skills/adeo-nuxt-ui/SKILL.md once; reuse existing UButton and UTextarea patterns. Do not explore installed Nuxt UI internals or type catalogs before implementing; let the actual typecheck identify any API mismatch. Implement the requested behavior and focused persistence/isolation tests, then move to verify_work.
3. Do not edit agent/policy/workflow files, factory/CONTRACT.md, package scripts, dependencies, routing or validation configuration. Those require a separate reviewed factory change and publication rejects them. Do not change the rules judging your own work. Scope concerns or missing dependencies belong in limitations, not an infrastructure detour.
4. Call verify_work after the final edits. It records actual typecheck, tests and build results and binds them to the source changes; it also reproduces one named required check on the pristine \`/workspace/base\` copy. Use the \`baseCommand\` input when a check other than the default \`pnpm test\` best demonstrates the task. If a check fails, inspect that failure, repair within the task and verify again. Do not claim passing checks from assertions or old evidence.
