...
const browserEvidence = z.object({ complete: z.boolean(), observations: z.array(z.object({ origin: z.string(), headSha: sha, sessionId: z.string(), source: z.enum(["base", "head"]).optional(), snapshot: z.boolean(), interaction: z.boolean(), keyboard: z.boolean(), screenshot: z.boolean(), afterInteraction: z.boolean(), route: z.string().optional(), eventIds: z.array(z.string()).max(50) })).max(4) });
const reviewPublication = z.object({ body: z.enum(["published", "not_required", "failed"]), githubReview: z.enum(["published", "already_published", "failed"]), review: z.object({ id: z.number().int().positive(), url: z.string().url().optional(), deduplicated: z.boolean() }).optional(), errors: z.array(z.string()).max(10) });
const workerResult = z.object({ station: z.literal("worker"), operationId: z.string().uuid().optional(), sessionId: z.string(), revision: sha, summary: z.string(), publication: z.object({ branch: z.string(), number: z.number().int().positive(), url: prUrl, headSha: sha, baseSha: sha, ownerSessionId: z.string().optional(), targetBranch: z.string().optional(), targetHeadSha: sha.optional(), parentPrNumber: z.number().int().positive().optional() }), commands: z.array(command), browserEvidence: browserEvidence.optional() });
const reviewerResult = z.object({ station: z.literal("reviewer"), sessionId: z.string(), prNumber: z.number().int().positive(), url: prUrl, baseSha: sha, headSha: sha, targetBranch: z.string().optional(), verdict: z.enum(["approve", "changes_requested", "incomplete"]), summary: z.string(), findings: z.array(z.object({ severity: z.enum(["blocking", "nonblocking"]), path: z.string(), line: z.number().int().positive().optional(), message: z.string(), evidence: z.string() })), commands: z.array(command), limitations: z.array(z.string()), browserEvidence: browserEvidence.optional(), visualReview: visualReviewPacketSchema.optional(), publication: reviewPublication.optional(), capturedAt: z.string() });
