...
const deliveryPhaseLabels: Record<string, string> = {
  worker_starting: "Starting worker",
  working: "Worker is building",
  review_starting: "Starting reviewer",
...
function deliveryStageStatus(stage: DeliveryStage, input: DeliveryFlowInput, visited: Set<DeliveryStage>): FlowStatus {
  ...
  const activeStage = stageForPhase[phase];
...
export function deliveryFlow(input: DeliveryFlowInput): FlowModel {
  ...
  const nodes: FlowNodeSpec[] = [
    ...
    },
    {
      ...
      subtitle: "Checks exact revision",
    ...
    },
    {
      ...
      position: { x: 478, y: 292 },
      detail: input.cycle ? "The existing branch owner receives the review findings and keeps the same branch." : "Blocking findings can send work back to its existing owner.",
      meta: input.phase === "needs_revision" ? "Review findings need a response" : undefined,
    },
    {
      id: "merge",
      title: "Merge gate",
      subtitle: "Independent evidence",
      status: deliveryStageStatus("merge", input, visited),
      icon: "i-lucide-git-merge",
      position: { x: 716, y: 126 },
      detail: "The host checks the exact review, repository checks and target head before deciding.",
  ...
  return { nodes, edges, activeNodeId: phase ? stageForPhase[phase] : "request" };
}
...
export function stationFlow(input: StationFlowInput): FlowModel {
  const stationName = input.station === "worker" ? "Worker" : "Reviewer";
  const resultTitle = input.station === "worker" ? "Draft PR" : "Review verdict";
  const resultDetail = input.result?.summary || (input.station === "worker" ? "The published PR will appear here." : "The exact-head review will appear here.");
  const maxTools = 6;
