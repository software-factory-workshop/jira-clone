...
export function useCockpit() {
 ...
 async function refresh(collection:Collection){const response=await $fetch<{items:CockpitRecord[]}>(`/factory/cockpit/${collection}`);items.value[collection]=response.items;return response.items;}
 async function save(collection:Collection,id:string,value:Record<string,unknown>,version=0){const response=await $fetch<{item:CockpitRecord}>(`/factory/cockpit/${collection}/${encodeURIComponent(id)}`,{method:'PUT',body:{value,expectedVersion:version},retry:0});items.value[collection]=[response.item,...items.value[collection].filter(r=>r.id!==id)];return response.item;}
 async function remove(collection:Collection,item:CockpitRecord){await $fetch(`/factory/cockpit/${collection}/${encodeURIComponent(item.id)}`,{method:'DELETE',body:{expectedVersion:item.version},retry:0});items.value[collection]=items.value[collection].filter(r=>r.id!==item.id);}
