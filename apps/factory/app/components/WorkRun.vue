...
const label = computed(() => result.value ? result.value.station === "worker" ? "Draft PR created" : result.value.verdict === "approve" ? "Review passed" : result.value.verdict === "changes_requested" ? "Changes requested" : "Review incomplete" : needsDecision.value ? "Awaiting decision" : stopped.value ? "Stopped" : awaitingChild.value ? "Station dispatched" : authorizations.value.length ? "Connection needed" : active.value ? "Running" : ended.value ? "Incomplete" : "Disconnected");
...
<template>
  <div :class="{ 'panel station-run': !child }">
    <template v-if="!childId">
      ...
      <p v-if="awaitingChild" role="status">Waiting for the {{ station === 'worker' ? 'worker' : 'reviewer' }} session. The task has been dispatched.</p>
