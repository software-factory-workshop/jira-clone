...
const agents = ['task-miner', 'worker', 'reviewer'];
...
for (const prefix of ['/eve/v1', '/factory']) {
  assert(authored.rewrites.some(route => route.source === `${prefix}/(.*)` && route.destination?.service === 'task-miner'), `Missing compatibility route for ${prefix}`);
}
