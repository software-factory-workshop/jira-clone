...
const outputs = [...new Set([
  resolve(process.env.EVE_INTERNAL_BUILD_OUTPUT_DIRECTORY || (process.env.VERCEL ? '.vercel/output' : '.output')),
  // Eve hoists Workflow functions into the Nuxt host output before this script
  // resumes. Their copies need the same assets, including hidden route folders.
  ...(process.env.EVE_INTERNAL_HOST_BUILD_OUTPUT_DIRECTORY ? [resolve(process.env.EVE_INTERNAL_HOST_BUILD_OUTPUT_DIRECTORY)] : []),
])];
let copied = 0;
for (const output of outputs) {
 ...
 assert(copied > before, `No ACP bundle found in ${output}; recheck the hosted packaging contract.`);
}
