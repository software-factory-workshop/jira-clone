...
export async function readStationRun(attachSession:(id:string)=>Parameters<typeof readRun>[0],id:string,filter:Parameters<typeof readRun>[1]={}) {
 const parent=await readRun(attachSession(id),filter);
 if(!parent.complete||!parent.childSessionId||filter.deliveryId)return {...parent,sessionId:id};
 const child=await readRun(attachSession(parent.childSessionId),{operationId:filter.operationId});
