...
export function hostResult(events:unknown[],tool:string,sessionId:string,operationId?:string){
 for(const event of [...events].reverse()) {const parsed=resultEvent.safeParse(event);if(!parsed.success||parsed.data.data.result.toolName!==tool)continue;const output=z.object({sessionId:z.literal(sessionId),operationId:z.string().optional()}).passthrough().safeParse(parsed.data.data.result.output);if(output.success&&(!operationId||output.data.operationId===operationId))return output.data;}
}
