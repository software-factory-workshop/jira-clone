...
export function resumeMessage(operationId:string){return `Factory recovery ${operationId}. Continue your original authenticated task in the existing workspace. Preserve all pending changes and original scope. If repository checks were blocked by factory context, use refresh_target to incorporate the current target without losing your work, then run verify_work and publish_work. Do not bypass checks or create a replacement owner.`;}
export function resumeReceipt(events:unknown[],operationId:string){for(const event of events){const parsed=z.object({type:z.literal('message.received'),data:z.object({message:z.literal(resumeMessage(operationId))}),meta:z.object({deliveryIds:z.array(z.string()).min(1)})}).safeParse(event);if(parsed.success)return parsed.data.meta.deliveryIds[0];}}
