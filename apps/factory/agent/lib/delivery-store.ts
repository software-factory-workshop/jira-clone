...
export async function readDelivery(id:string){const response=await get(path(id),{access:'private',useCache:false});if(!response)return null;if(response.statusCode!==200||!response.stream)throw new Error('Delivery storage unavailable');return{state:await new Response(response.stream).json() as Delivery,etag:response.blob.etag};}
