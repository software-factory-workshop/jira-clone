...
export default defineHook({events:{async 'session.started'(_,ctx){
 ...
 await updateCockpit(doc=>doc.runs[ctx.session.id]??changeRecord(doc,'runs',ctx.session.id,{station,label:request&&'title'in request?request.title:station==='mining'?'Repository investigation':'PR review',...(request&&'operationId'in request?{operationId:request.operationId}:{})},0));
}}});
