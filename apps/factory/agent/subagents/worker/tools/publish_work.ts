...
import { requireStation,stationRequest,workerRequest } from "../../../lib/station-access";
...
export default defineTool({description:"Publish verified source changes as one draft pull request on a host-chosen feature branch. No merge. Protected policy/agent/workflow files cannot be published.",inputSchema:z.object({summary:z.string().min(10).max(3000),limitations:z.array(z.string()).max(10)}).strict(),
 async execute(input,ctx){
  ...
  if(!publication){
   ...
   publication=await publishWork(token,{sessionId:ctx.session.id,baseSha:state.revision,title:request.title,body:`${input.summary}\n\n## Requested outcome\n${request.brief}\n\n## Validation\n${state.commands.slice(-3).map(c=>`- ${c.command}: exit ${c.exitCode}`).join("\n")}\n\n## Limitations\n${input.limitations.join("\n")||"None reported."}\n\nNative Eve session: ${ctx.session.id}\nSource: ${state.revision}`,changes},ctx.abortSignal);
   workState.update(s=>({...s,publication,recorded:true}));
  }
  return{station:"worker" as const,sessionId:ctx.session.id,revision:state.revision,publication,summary:input.summary,limitations:input.limitations,commands:state.commands,capturedAt:new Date().toISOString()};
 }});
