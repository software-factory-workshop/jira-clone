...
export default defineTool({description:"Refresh your own workspace against its latest target branch with a three-way merge. Preserves your source edits, exposes real conflict markers for resolution, and invalidates previous checks. Never writes the target branch.",inputSchema:z.object({}).strict(),
 async execute(_,ctx){
  ...
  const sandbox=await ctx.getSandbox();const changes=await collectChanges(sandbox,state.baseline,true,state.jiraManifest);
  ...
  workState.update(s=>({...s,prepared:true,revision:targetHead,targetHeadSha:targetHead,mergeTarget:!!s.publication,jiraManifest:jiraManifest(theirs.entries),baseline:manifestFor(theirs.entries).map(({file,sha256})=>({file,sha256})),verifiedDigest:null}));
