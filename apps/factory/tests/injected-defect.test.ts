...
test("lazy approval of the injected fixture stays human_review and manual", () => {
  ...
  state.publication = {
    number: 999,
    url: "https://github.com/software-factory-workshop/jira-clone/pull/999",
    headSha: sha,
  ...
  const decision = mergeDecision({
    files: [{ filename: "apps/jira/app/app.vue", status: "modified", patch: candidatePatch }],
    review: {
      ...lazyReview,
  ...
  assert.match(decision?.reason ?? "", /cosmetic CSS low-risk policy|incomplete|permitted browser exception/i);
});
