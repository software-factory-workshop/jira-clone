...
const proof = { type: 'action.result', data: { status: 'completed', result: { kind: 'tool-result', toolName: 'publish_work', output: { station: 'worker', sessionId: owner, revisionProtocol: 1, publication: { branch, number: 48, headSha: h, ownerSessionId: owner, targetBranch: 'main', targetHeadSha: a } } } } };
...
test('missing or legacy proof stays unavailable and points at a child PR, never a replacement', async () => {
  ...
  assert.match(stationLaunchError({ data: { error: { code: 'owner_unavailable', message: 'internal detail' } } }), /child PR/);
});
