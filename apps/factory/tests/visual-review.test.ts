...
import { factoryBlobPaths, factoryPorts } from "../runtime/lib/factory-config.ts";

const baseSha = "a".repeat(40);
const headSha = "b".repeat(40);
const digest = "c".repeat(64);
const jiraOrigin = `http://127.0.0.1:${factoryPorts.jira}`;
const frame = (phase: "before" | "after") => ({ phase, source: phase === "before" ? "base" as const : "head" as const, sourceSha: phase === "before" ? baseSha : headSha, url: `https://factory.example/${factoryBlobPaths.reviewArtifactsPrefix}packet?phase=${phase}`, sha256: digest, mediaType: "image/png" as const });

test("visual packets distinguish complete, partial and unrequired browser coverage", () => {
  const complete = buildVisualReviewPacket({ version: 1, requiredApps: ["jira"], baseSha, headSha, targetBranch: "main", reviewerSessionId: "wrun_reviewer", capturedAt: "2026-09-13T12:00:00.000Z", artifacts: [{ id: "a".repeat(32), app: "jira", origin: jiraOrigin, route: "/issues", baseSha, headSha, targetBranch: "main", capturedAt: "2026-09-13T12:00:00.000Z", before: frame("before"), after: frame("after") }], limitations: [] });
...
test("visual PR section is deterministic, bound to the candidate, and replaceable", () => {
  const packet = buildVisualReviewPacket({ version: 1, requiredApps: ["jira"], baseSha, headSha, targetBranch: "main", reviewerSessionId: "wrun_reviewer", capturedAt: "2026-09-13T12:00:00.000Z", artifacts: [{ id: "a".repeat(32), app: "jira", origin: jiraOrigin, route: "/issues?filter=open", baseSha, headSha, targetBranch: "main", capturedAt: "2026-09-13T12:00:00.000Z", before: frame("before"), after: frame("after") }], limitations: [] });
