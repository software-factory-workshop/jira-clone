...
test("the worker tool, built root and answer route share the durable owner-input contract", async () => {
  const tool = await source("runtime/stations/worker/tools/ask_owner.ts");
  const reexport = await source("agents/worker/agent/tools/ask_owner.ts");
  ...
  assert.match(reexport, /runtime\/stations\/worker\/tools\/ask_owner/);
