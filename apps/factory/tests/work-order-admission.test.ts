...
test("delivery requires explicit approval of mined admission and preserves all outcomes", async () => {
  const delivery = await source("runtime/channels/delivery.ts");
  const cockpit = await source("runtime/channels/cockpit.ts");
  ...
  assert.match(delivery, /approved after task mining/);
  assert.match(miningRun, /Approve and start work order/);
  assert.match(miningRun, /Reject and edit request/);
  assert.match(miningRun, /"\/factory\/cockpit\/approve"/);
  assert.match(cockpit, /binding\.value\.draftId!==body\.draftId/);
  assert.match(cockpit, /admission\.data\.kind!==['"]work_order['"]/);
