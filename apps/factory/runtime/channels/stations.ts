...
export async function stationOperation(request:Request,{from,params,resolveSession,attachSession}:RouteHandlerArgs,addressPrefix?:string){
  ...
  if(station==="revisions") {
   try {
    ...
    return Response.json({sessionId:ownerId,ownerSessionId:ownerId,station:"worker",execution:"owner",rootAgent:root,operationId:revision.operationId,deliveryId:accepted.deliveryId},{status:202});
  ...
  return Response.json(await rootRequest(station,'/factory/run',{request:parsed.data,contextKey:addressPrefix}),{status:202});
}
