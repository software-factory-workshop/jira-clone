...
export default defineTool({description:"Prepare a clean pinned main snapshot, frozen dependencies and the authenticated task brief. Call first.",inputSchema:z.object({}),
 async *execute(_,ctx){
  ...
  if(prior.completedOperations?.[operationId]){yield{phase:"Already published",result:prior.completedOperations[operationId]};return;}
  if(prior.prepared&&prior.operationId===operationId){yield{phase:"Prepared",revision:prior.revision,request:{...original,brief:prior.activeBrief},targetBranch:prior.targetBranch};return;}
