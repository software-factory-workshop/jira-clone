...
import { GitHubError, loadPullRequest } from "../../../lib/work-github";
...
function failureDetails(error: unknown) {
  const message = (error instanceof Error ? error.message : String(error || "unknown preparation failure")).replace(/\s+/g, " ").trim().slice(0, 500);
  return { message, ...(error instanceof GitHubError && error.code ? { code: error.code } : {}), at: new Date().toISOString() };
}
...
export default defineTool({description:"Fetch the authenticated PR's exact base/head, independent candidate workspace, frozen dependencies, and baseline policy. Call first.",inputSchema:z.object({}),
 async *execute(_,ctx){
  ...
  try {
   ...
   workState.update(s=>({...s,prepared:setup.prepared,basePrepared:setup.basePrepared,revision:setup.revision,prepareAttempts:0,prepareFailure:null,jiraManifest:candidateManifest,jiraNuxtConfig:jiraNuxtConfig(pull.snapshot.entries),jiraLockfile:jiraLockfile(pull.snapshot.entries),baseline:setup.files.map(({file,sha256})=>({file,sha256})),commands:setup.commands,pull:metadata,contextGaps:[...setup.contextGaps,...pull.contextGaps,...setup.baseContextGaps]}));
   ...
   workState.update(s=>({...s,prepareAttempts:attempt,prepareFailure:failureDetails(error)}));
   throw error;
  }
 },
