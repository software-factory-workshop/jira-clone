...
export default defineTool({description:"Independently run mandatory typecheck, tests and build on the unchanged pinned PR candidate. Required before an approve verdict.",inputSchema:z.object({}),
 async *execute(_,ctx){
  ...
  workState.update(s=>({...s,reviewVerified:true}));
  log.set({factory:{station:"reviewer",stage:"verify_review",outcome:"passed",checkCount:verificationCommands(!!state.pull?.files.some(f=>f.filename.startsWith("apps/jira/"))).length,headSha:state.pull?.headSha}});
  yield{phase:"Candidate verified",headSha:state.pull?.headSha};
 }});
