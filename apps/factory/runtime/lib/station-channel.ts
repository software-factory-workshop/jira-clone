...
import { workerRequest,reviewerRequest,revisionRequest,stationAddress,type Station } from './station-access';
...
export function stationChannel(station:Station) {
 return defineChannel({routes:[
  POST('/factory/run',async(request,{from,resolveSession})=>{
   ...
   const input=(station==='worker'?workerRequest:reviewerRequest).parse(body.request);
