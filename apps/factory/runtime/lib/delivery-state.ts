...
export type ExecutionStation = 'worker' | 'reviewer' | 'revisions';
...
export function newDelivery(principalId: string, request: DeliveryRequest): Delivery {
  ...
  const operationId = operationFor(id, 'worker', 0);
...
export function applyReview(state: Delivery, review: NonNullable<Delivery['review']>) {
  ...
  if (review.findings.some(finding => finding.severity === 'blocking') && state.cycle < state.request.maxRevisions) {
    ...
    return beginRevision(state, operationId, `Original task remains in force. Fix the independent review findings, preserve all original requirements, run checks, and update your existing PR.\n${JSON.stringify(review.findings)}`, {
      reason: 'Independent review found blocking issues; the original owner gets a bounded revision.',
    });
  }
  if (review.verdict === 'approve' && !review.limitations.length && !review.findings.some(finding => finding.severity === 'blocking')) {
    return transition(state, 'ready', { reason: 'Independent review approved the published candidate.' });
  }
  return transition(state, 'human_review', { reason: 'Independent review requires a human decision.' });
}
