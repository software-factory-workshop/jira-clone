...
function contextType(action: (typeof FACTORY_ACTION_NAMES)[number]): string {
  return `{ input: ${factoryInputRecordType(action)}, expectedRevision: String, candidateSha: String, baseSha: String, verifiedSha: String, reviewedSha?: String, branch: String, lane: String, budget: Long, riskClass: String, evidence?: Evidence, approval?: Approval }`;
}
